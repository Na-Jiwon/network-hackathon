...
| 기간 | 2023.07 ~ 09 (3개월) |
| 인원 | 4인 팀 |
| 역할 | EDA에서 제조사별 도메인 시프트 원인 분석, 정규화 전략 설계 및 86개 매핑 규칙 정의·적용, ticketno 시퀀스 구조 설계 |
...
- **전처리가 모델 선택보다 성능에 직접적이었다**: 제조사 3곳의 경보 표현을 하나씩 대조하며 86개 매핑 규칙을 수작업으로 만들었다. 모델 아키텍처를 바꾸는 것보다 모델이 학습할 수 있는 형태로 데이터를 변환하는 것이 선행되어야 한다는 걸 체감했다.

- **수작업의 한계가 다음 프로젝트의 출발점이 됐다**: 이 방식은 효과적이었지만 새 제조사가 추가될 때마다 같은 수작업을 반복해야 한다는 한계가 있었다. 이 경험이 LLM + RAG로 수동 규칙 없이 분류하는 [alarm-rag-agent](https://github.com/Na-Jiwon/network-hackathon/tree/main/alarm-rag-agent) 프로젝트의 출발점이 됐다.

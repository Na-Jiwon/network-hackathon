...
- 기존 프로젝트에서 86개 매핑 규칙을 수작업으로 정의해야 하는 한계를 확인했다.
- 새로운 제조사가 추가될 때마다 수작업이 반복되는 문제가 발생한다.
- → 이를 해결하기 위해 경보 메시지 정규화를 자동화하는 RAG 기반 Agent를 설계했다.
- 
---
...
- Retrieval 단계: 유사 경보 메시지 검색
- Generation 단계: 표준화된 표현 생성
- Agent 역할: 정규화 규칙을 동적으로 생성
